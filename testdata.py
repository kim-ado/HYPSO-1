...
def resize_images(img_ms, img_pan, ratio, sensor=None, mtf=None, apply_mtf_to_pan=False):
    ...
    # Convert the filter to 3D for channel-wise multiplication
    h = np.expand_dims(h, axis=0)

    I_MS_LP = np.zeros_like(img_ms, dtype='float32')

    for i in range(img_ms.shape[-1]):
        I_MS_LP[..., i] = np.convolve(img_ms[..., i], h[0, 0, i], mode='same', boundary='symm')
    ...
    if apply_mtf_to_pan:
        ...
        I_PAN_LP = np.convolve(img_pan[0, 0, :], h[0, 0, 0], mode='same', boundary='symm')
